...
export default class TeamGrantAdminSchedule extends Command {
  static description = "Schedule a grantTeamRole(ADMIN) operation for a timelocked app through its Timelock";

  static args = {
    address: Args.string({
      description: "Address to grant the ADMIN role to",
      required: true,
    }),
  };
  ...
  static flags = {
    ...
    after: Flags.string({
      required: true,
      description: "Delay before operation can execute (e.g. 30s, 5m, 2h, 1d)",
      env: "ECLOUD_OP_DELAY",
    }),
  };
  ...
  async run() {
    return withTelemetry(this, async () => {
      const { args, flags } = await this.parse(TeamGrantAdminSchedule);
      ...
      const appId = await getOrPromptAppID({ appID: flags.app, environment, privateKey, rpcUrl, action: "schedule grant ADMIN" });
      ...
      if (isMainnet(environmentConfig)) {
        const confirmed = await confirm("Schedule this ADMIN grant?");
        if (!confirmed) {
      ...
      const { tx } = await compute.app.scheduleTimelockGrantAdmin(appId, timelockAddress, account, delaySeconds, {});

      this.log(`\n✅ ${chalk.green(`ADMIN grant scheduled (tx: ${tx})`)}`);
      this.log(chalk.cyan(`\nExecutable after: ${chalk.bold(readyDate)}`));
      this.log(chalk.cyan(`Run to execute:   ecloud compute team grant-admin execute ${account} --app=${appId} --timelock=${timelockAddress}`));
    });
  }
}
