# Ecloud Architecture Guide

## Summary

Ecloud enables developers to deploy **verifiable applications** in Trusted Execution Environments (TEEs). Each app receives its own deterministic wallet that serves as its cryptographic identity, allowing it to sign transactions, hold funds, and operate autonomously.

---

## Core Concept: Deterministic Application Identities

### How Ecloud Enables Autonomous Applications

When you deploy an app through Ecloud, the system provides:
...
sequenceDiagram
    ...
    Dev->>CLI: ecloud compute app deploy
...
**What happens:**
1. Developer runs `ecloud compute app deploy` with their application
...
sequenceDiagram
    ...
    Developer->>CLI: ecloud compute app deploy
    CLI->>CLI: Build Docker Image
...
```mermaid
stateDiagram-v2
    [*] --> Created: ecloud compute app create
    Created --> Deploying: ecloud compute app deploy
    Deploying --> Running: TEE boots successfully
    Running --> Upgrading: ecloud compute app upgrade (new version)
    Upgrading --> Running: Update complete
    Running --> Stopping: ecloud compute app stop
    Stopping --> Stopped: VM terminated
    Stopped --> Deploying: ecloud compute app start
    Running --> Terminated: ecloud compute app terminate
    Stopped --> Terminated: ecloud compute app terminate
    Terminated --> [*]
```
...
| Use Case | How Ecloud Enables It |
|----------|----------------------|
...
1. **Create**: `ecloud compute app create my-app typescript`
2. **Develop**: Write your application logic
3. **Test**: Run locally with test mnemonics
4. **Deploy**: `ecloud compute app deploy` (automatic encryption)
5. **Verify**: Check app addresses with `ecloud compute app info`
...
Ecloud provides a foundational primitive for blockchain applications: **deterministic, hardware-attested application identities**. This enables autonomous applications that can manage funds, sign transactions, and operate independently while remaining cryptographically verifiable.
